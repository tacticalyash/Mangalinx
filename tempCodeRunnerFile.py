...
class ImageAlbum:
    def __init__(self, root):
        ...
        todo_menu = tk.Menu(menu_bar, tearoff=0)
        todo_menu.add_command(label="Insert", command=self.insert_into_db)
        todo_menu.add_command(label="Open", command=self.open_from_db)
        menu_bar.add_cascade(label="Database", menu=todo_menu)
